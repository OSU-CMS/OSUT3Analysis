...
class getSiblings():
    ...
    def getDBSSiblings(self, output_json):
        ...
        for ifile, filename in enumerate(inputFiles):
            ...
            if args.totalEvents != -1 and self.numSharedEvents >= args.totalEvents: break
        ...
        if args.totalEvents != -1: sharedEvents = sharedEvents[:args.totalEvents]
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--totalEvents", type=int, help="Total number of events for jobs to run over (note this is a maximum but may return fewer if there are fewer events in filelist)", default=-1)
