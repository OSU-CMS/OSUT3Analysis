...
InfoPrinter::analyze (const edm::Event &event, const edm::EventSetup &setup)
{
  ...
  for (vector<edm::EventID>::const_iterator eventToPrint = eventsToPrint_.begin (); printAllEvents_ || eventToPrint != eventsToPrint_.end (); eventToPrint++)
    {
...
InfoPrinter::printEventFlags ()
{
  ...
  for (vector<bool>::const_iterator flag = cutDecisions->eventFlags.begin (); flag != cutDecisions->eventFlags.end (); flag++)
    {
...
InfoPrinter::printCumulativeObjectFlags ()
{
  ...
  for (map<string, vector<bool> >::const_iterator collection = cutDecisions->cumulativeObjectFlags.at (0).begin (); collection != cutDecisions->cumulativeObjectFlags.at (0).end (); collection++)
    collections.push_back (collection->first);
  sort (collections.begin (), collections.end ());
  !maxCutWidth_ && (maxCutWidth_ = getMaxWidth (cutDecisions->cuts));
  for (vector<string>::const_iterator collection = collections.begin (); collection != collections.end (); collection++)
    {
      ...
      ss_ << "--------------------------------------------------------------------------------" << endl;
      for (vector<map<string, vector<bool> > >::const_iterator cut = cutDecisions->cumulativeObjectFlags.begin (); cut != cutDecisions->cumulativeObjectFlags.end (); cut++)
        {
          ss_ << "\033[1;34m" << setw (maxCutWidth_) << left << cutDecisions->cuts.at (cut - cutDecisions->cumulativeObjectFlags.begin ()).name << "\033[0m";
          for (vector<bool>::const_iterator flag = cut->at (*collection).begin (); flag != cut->at (*collection).end (); flag++)
            {
...
InfoPrinter::printObjectFlags ()
{
  ...
  if (!cutDecisions->objectFlags.size ())
    return true;
  vector<string> collections;
  for (map<string, vector<bool> >::const_iterator collection = cutDecisions->objectFlags.at (0).begin (); collection != cutDecisions->objectFlags.at (0).end (); collection++)
    collections.push_back (collection->first);
  sort (collections.begin (), collections.end ());
  !maxCutWidth_ && (maxCutWidth_ = getMaxWidth (cutDecisions->cuts));
  for (vector<string>::const_iterator collection = collections.begin (); collection != collections.end (); collection++)
    {
      ...
      ss_ << "--------------------------------------------------------------------------------" << endl;
      for (vector<map<string, vector<bool> > >::const_iterator cut = cutDecisions->objectFlags.begin (); cut != cutDecisions->objectFlags.end (); cut++)
        {
...
InfoPrinter::printTriggerFlags ()
{
  ...
  for (vector<bool>::const_iterator flag = cutDecisions->triggerFlags.begin (); flag != cutDecisions->triggerFlags.end (); flag++)
    {
...
InfoPrinter::printVetoTriggerFlags ()
{
  ...
  for (vector<bool>::const_iterator flag = cutDecisions->vetoTriggerFlags.begin (); flag != cutDecisions->vetoTriggerFlags.end (); flag++)
    {
...
unsigned
InfoPrinter::getMaxWidth (const vector<string> &list)
{
  unsigned w = 0;
...
unsigned
InfoPrinter::getMaxWidth (const vector<Cut> &list)
{
  unsigned w = 0;
  for (vector<Cut>::const_iterator cut = list.begin (); cut != list.end (); cut++)
    {
