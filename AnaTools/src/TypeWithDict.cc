...
namespace anatools {
  ...
  TypeWithDict::byName(std::string const& name, long property) {
    ...
    if(name.back() == ']') {
      ...
      ret.arrayDimensions_ = edm::value_ptr<std::vector<size_t> >(new std::vector<size_t>);
    ...
    if(edm::stripNamespace(name) == "value_type") {
  ...
  TypeWithDict::TypeWithDict(std::type_info const& ti, long property /*= 0L*/) :
    ...
    char lastChar = edm::TypeID(*ti_).className().back();
    if (lastChar == '*' || lastChar == ']') {
      *this = TypeWithDict::byName(edm::TypeID(*ti_).className());
      return;
    }
  ...
  TypeWithDict::cppName() const {
    ...
    edm::replaceString(cName, "ULong64_t", "unsigned long long");
    edm::replaceString(cName, "Long64_t", "long long");
    return cName;
  }
  ...
  TypeWithDict::unscopedName() const {
    return edm::stripNamespace(name());
  }
  ...
  TypeWithDict::friendlyClassName() const {
    return edm::friendlyname::friendlyName(name());
  }
  ...
  TypeWithDict::toType() const {
    ...
    if(isArray()) {
      ...
      if(size == 1) {
        ...
        edm::value_ptr<std::vector<size_t> > emptyVec;
  ...
  operator==(TypeWithDict const& a, std::type_info const& b) {
    if(*a.ti_ == typeid(TypeWithDict::dummyType) || a.isPointer() || a.isArray()) {
      // No accurate type_info
      return a.name() == edm::TypeID(b).className();
    }
    return *a.ti_ == b;
  }
