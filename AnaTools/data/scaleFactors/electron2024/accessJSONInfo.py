...
def electronID_sf():
    ...
    for name in namesId:
        ...
        for eta in etaRange.keys():
            ...
            for pt in ptRange.keys():
                ptDict = {}
                valsf = myevaluator.evaluate("2024Prompt","sf",name, eta, pt)
                val_up = myevaluator.evaluate("2024Prompt","sfup",name, eta, pt)
                val_down = myevaluator.evaluate("2024Prompt","sfdown",name, eta, pt)
    ...
    fout = 'Run24_SF_ID.json'
    with open(fout, 'w+') as f:
        json.dump(new, f)

    files = ['Run24_SF_ID.json']
...
def electronReco_sf():
    ...
    fout = 'Run24_SF_RECO.json'
    with open(fout, 'w+') as f:
        json.dump(new, f)

    files = ['Run24_SF_RECO.json']
...
def combineRecoPlots():
    ...
    f_low = TFile.Open('egammaEffi.txt_EGM2D_lowPT.root', 'READ')
    f_med = TFile.Open('egammaEffi.txt_EGM2D_lowPT.root', 'READ')
    f_high = TFile.Open('egammaEffi.txt_EGM2D_highPT.root', 'READ')
