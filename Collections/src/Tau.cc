...
osu::Tau::passesLightFlavorRejection () const
{
  ...
  // againstMuonLoose3 still exists as a tau ID, but will follow the recommendation to use DeepTau
...
const bool 
osu::Tau::passesJetDiscrimination () const
{

  bool flag = true;
  #if DATA_FORMAT_IS_2022
  if (this->isTauIDAvailable("byVVVLooseDeepTau2018v2p5VSjet"))
    flag = flag && (this->tauID("byVVVLooseDeepTau2018v2p5VSjet") > 0.5);
  else if (this->isTauIDAvailable("byVVVLooseDeepTau2017v2p1VSjet"))
    flag = flag && (this->tauID("byVVVLooseDeepTau2017v2p1VSjet") > 0.5);
  else
  {
    edm::LogWarning ("osu_Tau") << "Tau ID \"byVVVLooseDeepTau2018v2p5VSjet\" and \"byVVVLooseDeepTau2017v2p1VSjet\" unavailable.";
    flag = flag && false;
  }
